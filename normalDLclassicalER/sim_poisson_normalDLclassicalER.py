...
if __name__ == '__main__':
    ...
    weights = J0*abs(np.random.randn(N,N))/(p*N)
    tau = par.tau
    b = par.b
    mu = par.mu
    gain = par.gain
    ...
    np.savetxt('weights_normalDLclassicalER_N' + str(N) + '_p' + str(p) + '_mu' + str(mu)+ '_J0' + str(J0)+ '_seed' + str(seed) + '.txt',W,'%.6f',' ','\n')

    spktimes, g_vec = sim_poisson(W, tstop, trans, dt)
    
    np.savetxt('spktimes_normalDLclassicalER_N' + str(N) + '_p' + str(p) + '_mu' + str(mu)+ '_J0' + str(J0)+ '_seed' + str(seed) + '.txt',spktimes,'%.6f',' ','\n')
    #np.savetxt('gvec.txt',g_vec,'%.6f',' ','\n')

    # compute some statistics
    mftratesfull = rates_ss(W)
    np.savetxt('mftratesfull_normalDLclassicalER_N' + str(N) + '_p' + str(p)+ '_mu' + str(mu)+ '_J0' + str(J0) + '_seed' + str(seed) + '.txt',mftratesfull,'%.6f',' ','\n')
