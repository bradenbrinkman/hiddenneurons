...
Network connectivity is a Gaussian-weighted network with Erdos-Reyni statistics and scaled by
...
if __name__ == '__main__':
    ...
    np.savetxt('weights_normalsignedbalancedER_N' + str(N) + '_p' + str(p) + '_mu' + str(mu)+ '_J0' + str(J0) + '_seed' + str(seed) + '.txt',W,'%.6f',' ','\n')

    spktimes, g_vec = sim_poisson(W, tstop, trans, dt)
    
    np.savetxt('spktimes_normalsignedbalancedER_N' + str(N) + '_p' + str(p) + '_mu' + str(mu)+ '_J0' + str(J0) + '_seed' + str(seed) + '.txt',spktimes,'%.6f',' ','\n')
