...
def bin_pop_array_spiketrain(spktimes,dt,dt_ccg,tstop,trans,ind_include):
    
    import numpy as np
    
    Nt = int(tstop/dt_ccg)+1
    
    spk_pop = np.zeros((Nt,ind_include.size))    
    
    count = 0
    for i in ind_include:
        spk_pop[:,count] = bin_spiketrain(spktimes, i, dt, dt_ccg, tstop, trans)
        count += count
